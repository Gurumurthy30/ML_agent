...
export default function ArtifactCard({ artifact }) {
  ...
  const handleDownload = () => {
    // Generate a mock downloadable file blob
    const content = artifact.content || `# Model Artifact: ${artifact.name}\n# Trained with LightGBM 5-Fold Stratified CV\n# Metrics: ${JSON.stringify(artifact.metrics || {})}\n`;
    const blob = new Blob([content], { type: 'application/octet-stream' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = artifact.filename || artifact.name || 'model_artifact.booster';
