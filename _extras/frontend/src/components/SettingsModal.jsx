...
export default function SettingsModal({ isOpen, onClose }) {
  const [model, setModel] = useState(() => localStorage.getItem('ollama_model') || 'qwen2.5-coder:32b');
  const [temperature, setTemperature] = useState(() => parseFloat(localStorage.getItem('ollama_temp') || '0.2'));
  const [maxTokens, setMaxTokens] = useState(() => parseInt(localStorage.getItem('ollama_tokens') || '4096'));
  ...
  const handleSave = () => {
    ...
    localStorage.setItem('ollama_temp', temperature.toString());
    localStorage.setItem('ollama_tokens', maxTokens.toString());
      ...
      <div className="bg-card border border-border rounded-2xl max-w-xl w-full p-6 shadow-2xl animate-in fade-in zoom-in-95 flex flex-col">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-border/80">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-[16px] font-semibold text-primary">Local Ollama Configuration</h2>
              <p className="text-xs text-muted">Configure local Ollama host endpoint, model tag or path, and inference parameters</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="py-4 space-y-4 text-xs">
          {/* Ollama Endpoint */}
          <div>
            <label className="block text-muted font-medium mb-1">Ollama Host URL / Endpoint</label>
            <input
              type="text"
              value={ollamaUrl}
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="text-muted font-medium">Model Name / Path</label>
              <div className="flex gap-1">
                {['qwen2.5-coder:32b', 'deepseek-r1:14b', 'llama3.3:70b'].map((preset) => (
                  <button
                    key={preset}
                    type="button"
                    onClick={() => setModel(preset)}
                    className="text-[10px] px-1.5 py-0.5 rounded bg-card hover:bg-border/60 text-muted hover:text-accent font-mono transition-colors cursor-pointer border border-border/40"
                  >
                    {preset.split(':')[0]}
                  </button>
                ))}
            ...
            <input
              ...
              placeholder="e.g. qwen2.5-coder:32b or custom model tag"
              className="w-full bg-[#181716] border border-border rounded-lg px-3 py-2 text-primary font-mono text-xs focus:outline-none focus:border-accent"
            />
            <p className="text-[10px] text-muted/70 mt-1">
              Enter any local Ollama model tag, GGUF path, or custom fine-tuned model.
            </p>
          </div>
